...
const char* ssidAP = "ESP32";  // Enter SSIDAP here
const char* passwordAP = "12345678";  //Enter PasswordAP here

/* MQTT settings*/
const char* ssid = "CasaWifi2.4";
...
void wifisetupAP(){
  ...
  Serial.print("Got IP: ");  Serial.println(ssidAP);
...
void loop() { 
  ...
  for (int i=1; i < 1000; i++) {
    ...
    mqttClient.publish("esp32/pressure", tempString);
    dtostrf(altitude, 1, 2, tempString);
    mqttClient.publish("esp32/altitude", tempString);
